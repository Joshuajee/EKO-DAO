...
describe("Governor Facet test", function () {
  ...
  GovernorFixture = async () => {
    ...
    const EkotokenInstance = await ekotoken.deploy(
    ...
    const GovernorFacet = await ethers.getContractAt(
      "GovernorFacet",
      diamondAddress
    );
  ...
  it("should be able to allow voting delegation on a particular proposal", async () => {
    const { GovernorFacet, owner, signer1, signer2, signer3 } =
      await loadFixture(GovernorFixture);

    await GovernorFacet.newProposal(Proposal_Name_1);

    await GovernorFacet.addVotingDelegate("1", signer1.address);
    await GovernorFacet.connect(signer2).addVotingDelegate(
      "1",
      signer3.address
    );
  ...
  it("should be able to remove voting delegate on a proposal", async () => {
    const { GovernorFacet, owner, signer1 } = await loadFixture(
      GovernorFixture
    );
    ...
    const delegateOneBefore = await GovernorFacet.viewDelegate(
      "1",
      owner.address
    );
    ...
    const delegateOneAfter = await GovernorFacet.viewDelegate(
      "1",
      owner.address
    );
    ...
    expect(await GovernorFacet.viewDelegate("1", owner.address)).to.equal(
  ...
  it("should be able to allow delegate vote for or against a proposal", async () => {
    ...
    await GovernorFacet.connect(signer1).addVotingDelegate(
      "1",
      signer3.address
    );
    await GovernorFacet.connect(signer1).addVotingDelegate(
    ...
    await GovernorFacet.connect(signer3).voteAgainstAsDelegate(
      "1",
      signer1.address
    );

    await GovernorFacet.connect(signer2).voteAgainstAsDelegate(
      "2",
      owner.address
    );
    await GovernorFacet.connect(signer3).voteForAsDelegate(
      "2",
      signer1.address
    );
