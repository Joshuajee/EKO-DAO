...
library LibCrowdFund {
  ...
  bytes32 constant ProjectStateLocation =
    keccak256(
      "admin.targetFund.TotalReacivedFund.MinimumContribution.amountWithraw.fundBalance.endDate"
    );
  bytes32 constant ProjectMappingLocation =
    keccak256("contributorAmountMapping");
  ...
  function getProjectMappingRecords()
    internal
    pure
    returns (ProjectMappings storage pm)
  {
  ...
  function getCrowdFundRecords()
    internal
    pure
    returns (CrowdFundingState storage cs)
  {
  ...
  function getCrowdFundMappingRecords()
    internal
    pure
    returns (CrowdFundingMapping storage cm)
  {
